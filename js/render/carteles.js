...
function rCDet(c, id) {
  ...
  c.innerHTML = `<div class="pbk" onclick="nav('carteles')">← Carteles</div>
    <div class="phero" style="border-left:4px solid var(--pur)"><button class="ed-btn" style="right:90px" onclick="event.stopPropagation();showModal('addVinculo',{tipo:'cartel',id:'${ct.id}'})">+ VINCULO</button><button class="ed-btn" onclick="showModal('editCartel','${ct.id}')">✏ EDITAR</button>
    ...
    <div class="ds"><h4>DESCRIPCION</h4><div class="df dg">${esc(ct.descripcion||'')}</div></div>
    ${ct.operaciones?`<div class="ds"><h4>OPERACIONES</h4><div class="df dg">${esc(ct.operaciones)}</div></div>`:''}
    ${renderVinculos('cartel', ct.id)}`;
}
