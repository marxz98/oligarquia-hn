...
function rDash(c) {
  ...
  c.innerHTML = `<div class="sh"><div><h2>Radiografia del Poder</h2><p>Base de datos ciudadana — Conectada a PostgreSQL en tiempo real</p></div></div>
    ...
    <div><h3 style="font-family:var(--fd);font-size:12px;color:var(--red);letter-spacing:2px;margin-bottom:10px;display:flex;align-items:center;gap:6px">${ICONS.siren} ALERTAS LEGALES</h3>
    ...
    <div><h3 style="font-family:var(--fd);font-size:12px;color:var(--amb);letter-spacing:2px;margin-bottom:10px;display:flex;align-items:center;gap:6px">${ICONS.star} SOCIOS PRINCIPALES (SAR)</h3>
    ...
    <h3 style="font-family:var(--fd);font-size:12px;color:var(--amb);letter-spacing:2px;margin:20px 0 10px;display:flex;align-items:center;gap:6px">${ICONS.building} GRUPOS FAMILIARES / ECONOMICOS</h3>
    <div class="grid g4">${DB.grupos.filter(g=>g.nombre!=='OTROS').map(g => {const co=GC[g.nombre]||'#6b7280';
      ...
      ${g.rankingPoder?`<div style="font-size:8px;color:var(--amb);margin-top:3px">${ICONS.star} Ranking SAR #${g.rankingPoder}</div>`:''}
      ${g.origenCapital==='Internacional'?`<div style="font-size:8px;color:var(--cyan);margin-top:2px">${ICONS.globe} ${esc(g.paisOrigen||'Internacional')}</div>`:''}
      </div>`}).join('')}</div>`;
}
