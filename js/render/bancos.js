...
function rBDet(c, id) {
  ...
  c.innerHTML = `<div class="pbk" onclick="nav('bancos')">← Bancos</div>
    <div class="phero" style="border-left:4px solid ${co}">
      ...
      ${g&&g.totalActivos?`<div style="font-size:11px;color:var(--amb);margin-top:4px">💰 Activos del grupo: L${(g.totalActivos/1000000).toLocaleString()}M (${g.pctPib}% PIB)</div>`:''}</div>
    </div>
    ${b.descripcion?`<div class="ds"><h4>DESCRIPCIÓN</h4><div class="df">${esc(b.descripcion)}</div></div>`:''}
    ${g?`<div class="ds"><h4>GRUPO CONTROLADOR</h4><div class="card" onclick="nav('g-det','${g.nombre}')" style="border-left:2px solid ${co}40">
      ...
      ${g.rankingPoder?`<div style="font-size:9px;color:var(--amb);margin-top:2px">⭐ Ranking SAR #${g.rankingPoder}</div>`:''}</div></div>`:''}
